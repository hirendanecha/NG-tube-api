...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.userName;
  let msg = `You have been assign in NudeGirls Tube channel by the NudeGirls Tube Admin.
  To access your channel, log into your NudeGirls Tube account,click on the
  NudeGirls Tube icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `NudeGirls Tube has approved your Connection application.`;
    } else {
      msg = `NudeGirls Tube has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
